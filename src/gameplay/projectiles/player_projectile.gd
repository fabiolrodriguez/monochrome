...
var _projectile_color := Color("4aa3ff")
...
func activate(spawn_transform: Transform2D, damage_scale: float, speed_scale: float, piercing: int, ricochets: int, is_critical: bool = false, projectile_color: Color = Color("4aa3ff")) -> void:
...
func _on_body_entered(body: Node2D) -> void:
	...
	if body.has_method("take_damage"):
		...
		AudioManager.play_sfx(&"critical_hit" if _is_critical else &"impact_hit", -20.0 if _is_critical else -26.0, 0.045)
	if _piercing_hits > 0:
		_piercing_hits -= 1
		return
...
func _draw() -> void:
	...
	var core_color := Color("ffd84a") if _is_critical else _projectile_color
	var trail_color := Color("fff0a6") if _is_critical else _projectile_color.lightened(0.55)
